...
/**
 * Query to fetch all attendance entries for the current user
 */
export function useGetAttendanceEntries() {
  const { actor, isFetching } = useActor();

  return useQuery<Array<[string, AttendanceDayEntry]>>({
...
export function useEditAttendanceEntry() {
  ...
  return useMutation({
    mutationFn: async ({
      date,
      entry,
    }: { date: string; entry: AttendanceDayEntry }) => {
