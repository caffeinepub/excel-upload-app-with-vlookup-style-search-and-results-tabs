...
import {
  ...
  exportHistoryToPdf,
} from "../lib/results/historyExport";
...
export function HistoryTab() {
  ...
  const filteredHistory = history.filter((item) => {
    ...
    if (
      searchKeyword.trim() &&
      !item.details.toLowerCase().includes(searchKeyword.toLowerCase())
    ) {
      return false;
    }
    return true;
  ...
  // Group by type
  const uploadHistory = history.filter(
    (item) => item.entryType === HistoryType.upload,
  );
  const searchHistory = history.filter(
    (item) => item.entryType === HistoryType.search,
  );
  const resultsHistory = history.filter(
    (item) => item.entryType === HistoryType.results,
  );
  const updateCheckingHistory = history.filter(
    (item) => item.entryType === HistoryType.updateChecking,
  );
  const budgetHistory = history.filter(
    (item) => item.entryType === HistoryType.budgetChange,
  );
  const expenseHistory = history.filter(
    (item) => item.entryType === HistoryType.expenseChange,
  );
        ...
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>
                Fetching your history from the backend...
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        ...
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>
                Failed to load history from backend
              </CardDescription>
            </div>
          </div>
        </CardHeader>
    ...
    <div className="space-y-6">
      <Card className="history-card">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <Button
                ...
                disabled={
                  clearHistoryMutation.isPending || history.length === 0
                }
                className="flex-shrink-0"
              >
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select value={selectedType} onValueChange={setSelectedType}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value={HistoryType.updateChecking}>
                    Update Checking
                  </SelectItem>
                  <SelectItem value={HistoryType.budgetChange}>
                    Budget
                  </SelectItem>
                  <SelectItem value={HistoryType.expenseChange}>
                    Expense
                  </SelectItem>
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              <span className="text-sm font-medium">Search Details</span>
              <Input
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-4">
            ...
            <div className="p-3 bg-muted/50 rounded-lg">
              ...
              <p className="text-lg sm:text-xl font-semibold">
                {updateCheckingHistory.length}
              </p>
            </div>
            <div className="p-3 bg-muted/50 rounded-lg">
              <p className="text-xs text-muted-foreground">Budget/Exp</p>
              <p className="text-lg sm:text-xl font-semibold">
                {budgetHistory.length + expenseHistory.length}
              </p>
            </div>
          </div>
            ...
            <div className="text-center py-12 text-muted-foreground">
              {history.length === 0 ? (
                <>
                  ...
                  <p>
                    No history entries yet. Start using the app to see your
                    activity here.
                  </p>
                </>
              ) : (
      ...
      <Card className="history-card">
        <CardHeader>
          ...
          <CardDescription>
            Browse history entries grouped by function type
          </CardDescription>
        </CardHeader>
        <CardContent>
                    ...
                    history.map((entry) => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <Badge
                            variant={getTypeBadgeVariant(entry.entryType)}
                            className="text-xs history-badge"
                          >
                            {getTypeLabel(entry.entryType)}
                          </Badge>
