 ...
 */
export function secondsToHoursMinutes(seconds: bigint): {
  hours: number;
  minutes: number;
} {
...
export function validateAttendanceConfig(
  ...
  const hours = Number.parseInt(regularHours, 10);
  const minutes = Number.parseInt(regularMinutes, 10);
  ...
  const leaveDays = Number.parseInt(leavePolicy, 10);
  if (Number.isNaN(leaveDays) || leaveDays < 0 || leaveDays > 365) {
    return {
      valid: false,
      error: "Leave policy must be between 0 and 365 days",
    };
  }
