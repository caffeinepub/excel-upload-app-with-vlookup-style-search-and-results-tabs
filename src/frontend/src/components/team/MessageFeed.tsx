...
function MessageBubble({
  ...
  return (
    <div
      className={`flex gap-2 items-start group ${isOwn ? "flex-row-reverse" : "flex-row"}`}
    ...
    >
      ...
      >
        <div className="flex items-baseline gap-2 mb-1">
          ...
          {canDelete && hovered && onDeleteMessage && (
            <Button
              variant="ghost"
              size="icon"
              className="h-5 w-5 text-destructive/60 hover:text-destructive hover:bg-destructive/10 ml-1"
              onClick={() => onDeleteMessage(msg.id)}
              data-ocid={`team.dm.delete_button.${markerIndex}`}
            >
