...
import type { SearchParams, SheetData } from "../../state/appState";
...
export function VlookupAnimation({
  data,
  searchParams,
  onClose,
}: VlookupAnimationProps) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < Math.min(data.rows.length, 20); i++) {
      ...
      if (searchParams.matchType === "exact") {
        ...
        if (!Number.isNaN(numValue) && !Number.isNaN(lookupNum)) {
  ...
  const getCellHighlight = (rowIndex: number, colIndex: number): string => {
    ...
    if (
      currentStepData.type === "highlight-lookup" &&
      colIndex === keyColumnIndex
    ) {
      return "bg-blue-200 dark:bg-blue-900/50 animate-pulse";
    }

    if (
      currentStepData.type === "scan-row" &&
      currentStepData.rowIndex === rowIndex
    ) {
    ...
    if (
      currentStepData.type === "return-value" &&
      currentStepData.rowIndex === rowIndex
    ) {
      if (colIndex === keyColumnIndex) {
        return "bg-green-300 dark:bg-green-700/70";
    ...
    if (currentStepData.type === "complete") {
      const foundStep = steps.find((s) => s.type === "found") as
        | { type: "found"; rowIndex: number }
        | undefined;
