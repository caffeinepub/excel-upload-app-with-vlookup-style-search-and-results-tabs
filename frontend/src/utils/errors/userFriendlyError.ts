...
export function getUserFriendlyError(error: unknown): string {
  ...
  if (error instanceof Error) {
    ...
    if (lowerMessage.includes('actor not available') || lowerMessage.includes('actor not ready')) {
      return 'Connecting to backend... Please try again in a moment.';
    }
    ...
    if ((lowerMessage.includes('trap') || lowerMessage.includes('rejected')) && !message.includes('Unauthorized:')) {
