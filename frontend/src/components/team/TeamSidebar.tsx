...
import { Channel, UserStatusEntry, UserStatusKind } from '../../backend';
import { useGetCallerUserProfile, useGetUserProfile } from '../../hooks/useUserProfile';
...
function statusColor(status: UserStatusKind | string): string {
  switch (status) {
    case UserStatusKind.online: return 'bg-green-500';
    case UserStatusKind.away: return 'bg-yellow-500';
    case UserStatusKind.busy: return 'bg-red-500';
...
function UserDmItem({ user, isSelected, onlineUsers, onClick }: UserDmItemProps) {
    ...
    <div className="flex flex-col h-full bg-sidebar text-sidebar-foreground w-64 border-r border-sidebar-border">
      ...
      <div className="flex items-center gap-2 px-3 py-3 border-b border-sidebar-border">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-sidebar-foreground/60 truncate">Team Chat</p>
        </div>
      </div>
        ...
        <div className="px-2 py-2 space-y-1">
          ...
          <Collapsible open={channelsOpen} onOpenChange={setChannelsOpen}>
            <CollapsibleTrigger asChild>
              <button className="flex items-center gap-1 w-full px-2 py-1 text-xs font-semibold uppercase tracking-wider text-sidebar-foreground/60 hover:text-sidebar-foreground transition-colors">
                {channelsOpen ? (
          ...
          <Collapsible open={dmsOpen} onOpenChange={setDmsOpen}>
            <CollapsibleTrigger asChild>
              <button className="flex items-center gap-1 w-full px-2 py-1 text-xs font-semibold uppercase tracking-wider text-sidebar-foreground/60 hover:text-sidebar-foreground transition-colors mt-2">
                {dmsOpen ? (
                  <ChevronDown className="h-3 w-3" />
                ) : (
                  <ChevronRight className="h-3 w-3" />
          ...
          <Collapsible open={onlineOpen} onOpenChange={setOnlineOpen}>
            <CollapsibleTrigger asChild>
              <button className="flex items-center gap-1 w-full px-2 py-1 text-xs font-semibold uppercase tracking-wider text-sidebar-foreground/60 hover:text-sidebar-foreground transition-colors mt-2">
                ...
                Online ({onlineUsers.filter((u) => u.status === UserStatusKind.online).length})
              </button>
            </CollapsibleTrigger>
              ...
              {onlineUsers
                ...
                .map((u) => (
                  <div
                    ...
                    className="flex items-center gap-2 px-2 py-1.5 text-sm text-sidebar-foreground/80"
                  >
                    <div className="relative flex-shrink-0">
                      <Circle className="h-5 w-5 text-sidebar-foreground/30" />
                      <span
                        className={`absolute inset-0 m-auto h-2 w-2 rounded-full ${statusColor(u.status)}`}
                      />
                    </div>
                    <span className="truncate text-xs">{u.principal.toString().slice(0, 12)}…</span>
                  </div>
                ))}
            </CollapsibleContent>
          </Collapsible>
