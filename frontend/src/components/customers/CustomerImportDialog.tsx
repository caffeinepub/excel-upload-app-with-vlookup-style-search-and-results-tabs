...
function normalizeKey(key: string): string {
  ...
  if (['name', 'fullname', 'customername', 'clientname'].includes(k)) return 'name';
  if (['email', 'emailaddress', 'mail'].includes(k)) return 'email';
  if (['phone', 'phonenumber', 'mobile', 'contact', 'tel', 'telephone'].includes(k))
    return 'phoneNumber';
  if (['address', 'addr', 'location'].includes(k)) return 'address';
  if (['company', 'organization', 'org', 'business', 'firm'].includes(k)) return 'company';
  if (['workdetails', 'work', 'details', 'notes', 'description', 'service'].includes(k))
    return 'workDetails';
  return k;
}
...
async function loadSheetJS(): Promise<XLSXLib> {
  return new Promise((resolve, reject) => {
    ...
    script.onload = () => resolve((window as unknown as Record<string, unknown>).XLSX as XLSXLib);
...
async function loadPDFJS(): Promise<PDFLib> {
  return new Promise((resolve, reject) => {
    ...
    script.onload = () => {
      const lib = (window as unknown as Record<string, unknown>).pdfjsLib as PDFLib;
  ...
  for (const line of lines) {
    ...
    if (parts.length >= 2) {
      if (!headerRow) {
        ...
        if (normalized.some((k) => ['name', 'email', 'phoneNumber'].includes(k))) {
  ...
  if (customers.length === 0) {
    ...
    for (const line of lines) {
      ...
      if (match) {
        ...
        if (
          ['name', 'email', 'phoneNumber', 'address', 'company', 'workDetails'].includes(key)
        ) {
...
export default function CustomerImportDialog({ open, onOpenChange, existingCustomers }: Props) {
  ...
  parsedCustomers.forEach((c, i) => {
    const isDup = existingCustomers.some(
      (ex) =>
        (c.phoneNumber.trim() !== '' && ex.phoneNumber === c.phoneNumber.trim()) ||
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-3xl w-full max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload an Excel (.xlsx, .xls), CSV, or PDF file to import customer records. The file
            should have column headers like: Name, Email, Phone, Company, Address, Work Details.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="space-y-2">
              ...
              {duplicateIndices.size > 0 && (
                <p className="text-xs text-muted-foreground">
                  Records matching an existing phone number or email will be skipped.
                </p>
              )}
            </div>
          )}
