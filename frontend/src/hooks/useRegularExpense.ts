...
export function useSaveBudget() {
  ...
  return useMutation({
    mutationFn: async (budget: {
      ...
      if (!monthlyLimitResult.success) throw new Error(`Monthly limit: ${monthlyLimitResult.error}`);

      const dayLimitResult = parseNatBigInt(budget.dayLimit);
      if (!dayLimitResult.success) throw new Error(`Daily limit: ${dayLimitResult.error}`);

      const savingsGoalResult = parseNatBigInt(budget.savingsGoal);
      if (!savingsGoalResult.success) throw new Error(`Savings goal: ${savingsGoalResult.error}`);
...
export function useAddExpense() {
  ...
  return useMutation({
    mutationFn: async (expense: {
      ...
      if (!amountResult.success) throw new Error(`Amount: ${amountResult.error}`);
