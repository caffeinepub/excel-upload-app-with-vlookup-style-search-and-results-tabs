...
export function useGetAllHolidays() {
  ...
  return useQuery<HolidayEntry[]>({
    ...
    queryFn: async () => {
      ...
      return actor.getGlobalHolidays();
    },
...
/**
 * Mutation to create a new holiday (uses addGlobalHoliday)
 */
export function useCreateHoliday() {
  ...
  return useMutation({
    mutationFn: async ({ date, holidayType }: { date: string; holidayType: HolidayType }) => {
      ...
      await actor.addGlobalHoliday(date, holidayType);
    },
    onSuccess: () => {
...
/**
 * Mutation to update an existing holiday (remove + re-add)
 */
export function useUpdateHoliday() {
  ...
  return useMutation({
    mutationFn: async ({ date, holidayType }: { date: string; holidayType: HolidayType }) => {
      ...
      if (!actor) throw new Error('Backend connection not ready. Please wait a moment and try again.');
      if (isFetching) throw new Error('Backend is initializing. Please wait a moment and try again.');
      // Backend has no update; remove then re-add
      await actor.removeGlobalHoliday(date);
      await actor.addGlobalHoliday(date, holidayType);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['holidays'] });
...
/**
 * Mutation to delete a holiday (uses removeGlobalHoliday)
 */
